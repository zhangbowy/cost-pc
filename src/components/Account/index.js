    ...
    return (
      <Card style={{ width: width > 1280 ? '390px' : '336px',height:'220px',backgroundColor:'#fff',margin:'16px 16px 0 0'}} className={styles.cardContent} bordered={false}>
        ...
        <div style={{ margin:'10px 0'}} className={styles.bank}>
          ...
          {type===0?
            <span className={styles.bankNameBranch}>{accountCity()}<i style={{ color: 'rgba(216, 216, 216, 1)' }}><Icon type="line" /></i><Tooltip placement="topLeft" title={bankNameBranch}>{bankNameBranch}</Tooltip> </span>
            : empty()}
        </div>
        {/* 备注 */}

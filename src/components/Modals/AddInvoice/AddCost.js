...
import { Modal, Form, Input, Row, Col, Divider, InputNumber, Select, DatePicker, message, TreeSelect, Tree, Button, Cascader, Tooltip } from 'antd';
                  ...
                  item.note && item.type ?
                    item.note.length > 15 ?
                      <Tooltip title={item.note || '-'} getPopupContainer={triggerNode => triggerNode.parentNode}>
                        <span className="c-black-45 fs-12 m-l-8">{item.note}</span>
                      </Tooltip>
                      :
                      <span className="c-black-45 fs-12 m-l-8">{item.note}</span>
                      :
